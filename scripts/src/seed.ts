...
async function seed() {
    ...
    const hoursData = allLocations.flatMap((loc) => {
        ...
        return dayNames.map((_, dayIndex) => {
            ...
            // Louise Drive: Mon-Thu 7:30am-9pm, Fri 7:30am-10pm, Sat 9am-10pm, Sun 9am-9pm
            if (isLouise) {
                if (dayIndex === 0) return { locationId: loc.id, dayOfWeek: dayIndex, openTime: "09:00", closeTime: "21:00", isClosed: false };
                if (dayIndex === 5) return { locationId: loc.id, dayOfWeek: dayIndex, openTime: "07:30", closeTime: "22:00", isClosed: false };
            ...
            // Carlisle Pike & Carlisle Scoop Shop: Sun-Thu 12-9pm, Fri-Sat 12-10pm
            if (dayIndex === 5 || dayIndex === 6) return { locationId: loc.id, dayOfWeek: dayIndex, openTime: "12:00", closeTime: "22:00", isClosed: false };
            return { locationId: loc.id, dayOfWeek: dayIndex, openTime: "12:00", closeTime: "21:00", isClosed: false };
        });
    });
